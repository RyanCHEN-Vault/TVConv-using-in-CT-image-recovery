...
'''卷积层'''
class Conv_Block(nn.Module):  # 和Conv层代替pooling层后，就和VGGBlock一样样了
...
class UpSample(nn.Module):
    def __init__(self,channel):
        ...
        # channel//2输出量为输入的一半     1X1卷积核，步长为1   --> 降通道
        self.layer=nn.Conv2d(channel,channel//2,1,1)
    def forward(self,x,feature_map):
        # 最近邻插值法for填充扩大图像之后的空洞，scale_factor=2 --> 扩大2倍
...
class UNet(nn.Module):
    def __init__(self,num_classes):
        ...
        self.c1=Conv_Block(3,64)  # in_channel = 3, out_channel = 64
...
if __name__ == '__main__':
    # 检查网络结构
    x=torch.randn(2,3,256,256)  # n批次数=2（两张图片：segment+原图），3channel，256X256
    net=UNet()
    print(net(x).shape)    # 输出整个net跑完后的结构的结构，需要和原来（即x）一致